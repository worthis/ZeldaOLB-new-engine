...
void Event::update(const Uint8 *keys, bool capslock)
{
    ...
    current[kLShift] = keys[SDL_SCANCODE_LSHIFT];
    current[kRShift] = keys[SDL_SCANCODE_RSHIFT];
    current[kLCtrl] = keys[SDL_SCANCODE_LCTRL];
    current[kRCtrl] = keys[SDL_SCANCODE_RCTRL];
    ...
    current[QUIT_FORCED] = (current[kLAlt] || current[kRAlt]) && keys[SDL_SCANCODE_F4];
