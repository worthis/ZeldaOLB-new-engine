...
Sword::Sword(int i, bool ol, bool hasMask) : onilink(ol)
{
    force = i;
    if (onilink)
    {
        if (hasMask)
        {
            image = ResourceManager::getInstance()->loadImage("data/images/link/epeeOni2.png", true);
        }
        else
        {
...
bool Sword::hit(int offsetX, int offsetY, Direction dir, Animation anim, int step, int charge, int forceTotal)
{
    ...
    if (anim == HIT_EPEE)
    {
        ...
        MainController::getInstance()->getGameController()->getSceneController()->getScene()->getMap()->pickUpItems(&box);

        return MainController::getInstance()->getGameController()->getSceneController()->getScene()->testDegat(&box, dir, forceTotal, TA_SWORD, TE_NORMAL);
    }
    else if (anim == SPIN_EPEE)
    {
        ...
        {
        case N:
            if (step == 0 || step == 5 || (step >= 21 && step <= 26))
            {
                i = -5;
                j = 17;
                a = 8;
        ...
        tmp = MainController::getInstance()->getGameController()->getSceneController()->getScene()->testDegat(&box, dir, forceTotal * 2, TA_SWORD, TE_NORMAL);
        ...
        case 0:
            ...
            box.setX(i + offsetX);
            box.setY(j + offsetY);
            tmp = tmp || MainController::getInstance()->getGameController()->getSceneController()->getScene()->testDegat(&box, dir2, forceTotal * 2, TA_SWORD, TE_NORMAL);
            break;
        case 1:
            box.setW(1);
            box.setH(1);
    ...
    else if (charge)
    {
        ...
        return MainController::getInstance()->getGameController()->getSceneController()->getScene()->testDegat(&box, dir, forceTotal, TA_SWORD_HOLD, TE_NORMAL);
    }
    return false;
}
...
void Sword::drawCharge(int dstX, int dstY, Direction dir, bool move, int step, int charge)
{
    ...
    if (charge < 19)
    {
        ...
        {
        case N:
            dstX += (charge / 3) - 6 + ((charge + 2) % 3);
            dstY -= (charge / 2) - 5;
            break;
        case S:
            dstX -= (charge / 3) - 2 + ((charge + 2) % 3);
            dstY += (charge / 2);
            break;
        case W:
            dstY += ((charge + 2) % 3) - 1;
