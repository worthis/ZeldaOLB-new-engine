...
void Inventory::draw(Equipment e, int x, int y)
{
    ...
    case PALMES:
        ...
        break;
    case OCARINA:
        WindowManager::getInstance()->draw(inventaire, 0, 68, 16, 17, x, y);
        break;
    case MASQUE:
        WindowManager::getInstance()->draw(inventaire, 16, 136, 16, 17, x, y);
        break;
    case MASQUE_VIDE:
        WindowManager::getInstance()->draw(inventaire, 0, 136, 16, 17, x, y);
        break;
    case BOTTES:
        WindowManager::getInstance()->draw(inventaire, 48, 85, 16, 17, x, y);
        break;
    default:
        break;
    }
...
bool Inventory::hasObject()
{
    for (int i = 0; i < NB_EQPMT; i++)
    {
        if (i != CARTE && i != ENCYCLOPEDIE && equipment[i])
            return true;
    }
    return false;
}
