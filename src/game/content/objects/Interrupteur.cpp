...
void Interrupteur::loop()
{
    ...
    {
        if (map->checkCollisions(&bb, this, false, false, false, false, true) && !bb.intersect(link->getBoundingBox()))
        {
    ...
    else if (!used && usable && (!map->checkCollisions(&bb, this, false, false, false, false, true) || bb.intersect(link->getBoundingBox())))
    {
        if (type != 2)
            used = true;
        else
