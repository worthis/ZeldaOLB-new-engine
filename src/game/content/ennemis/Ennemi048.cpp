...
void Ennemi048::giveItem(int x, int y)
{
    Scene *scene = MainController::getInstance()->getGameController()->getSceneController()->getScene();
    BoundingBox *bounds = scene->getMap()->getBounds();
    if (scene->getMap()->getId() == 28 && bounds->getX() == 320 * 7 && bounds->getY() == 240 && !scene->getCoffre(14, 5))
    {
        AudioManager::getInstance()->playSound(TS_KILLENNEMY);
        Map *map = MainController::getInstance()->getGameController()->getSceneController()->getScene()->getMap();
        map->addEffect(new FumeeBlanche(x, y));
        scene->getMap()->addItem(ItemHelper::getInstance()->createItem(TI_CLE, x, y, 5, true));
    }
    else
    {
        Ennemi::giveItem(x, y);
