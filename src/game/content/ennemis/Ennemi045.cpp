...
void Ennemi045::ennLoop() {
    ...
    if (cooldown < 16) {
        ...
        switch (randomValue) {
            case 1 : if (x > limit.getX()) {moveX(-1);direction=W;} break;
            case 2 : if (x + width < limit.getX() + limit.getW()) {moveX(1); direction=E;} break;
            case 3 : if (y > limit.getY()) {moveY(-1);direction=N;} break;
            case 4 : if (y + height < limit.getY() + limit.getH()) {moveY(1); direction=S;} break;
            default :
                if (randomValue < 10) break;
                switch (direction) {
                    case N : if (y > limit.getY()) moveY(-1); break;
                    case S : if (y + height < limit.getY() + limit.getH()) moveY(1); break;
                    case W : if (x > limit.getX()) moveX(-1); break;
                    case E : if (x + width < limit.getX() + limit.getW()) moveX(1); break;
                }
                break;
        }
    }
