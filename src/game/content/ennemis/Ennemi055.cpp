...
Ennemi055::Ennemi055(int i, int j) : anim(0), animMax(1), vanim(180) {
    image = ResourceManager::getInstance()->loadImage("data/images/ennemis/ennemi55.png", true);
    chrono.reset();
...
void Ennemi055::draw(int offsetX, int offsetY) {
    if (!alive) {
        return;
    }
...
BoundingBox* Ennemi055::getBoundingBox() {
    box.setX(x + 3);
    box.setY(y + 10);
    return &box;
}
