...
TypeItem ItemRandomizer::checkWithLink(TypeItem type)
{
    ...
    case TI_MAGIE_BEAUCOUP:
        if (!link->getStatus()->getMaxMagic() || link->getStatus()->getMagic() >= link->getStatus()->getMaxMagic())
        {
            return TI_NO_ITEM;
        }
        break;
    case TI_BOMBE:
        if (!link->getInventory()->hasObject(BOMBES) || link->getStatus()->getBombs() >= link->getStatus()->getMaxBombs())
        {
            return TI_NO_ITEM;
        }
        break;
